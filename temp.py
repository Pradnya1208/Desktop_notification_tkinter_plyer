        ...
        def notify(t):
            ...
            if response == "yes":
                ...
                for m, CC, RC, DC in zip(merged.index, merged["Confirmed cases"], merged["Recovered cases"], merged["Death toll"]):
                    if "Maharashtra" in m:
                        #print(CC, RC, DC)
                          frame_notify = "Maharashtra => | Total : " + str(CC) + " | Deaths : "  + str(DC) + " | Recovered : " + str(RC)
   ...
   # City list on rightclick         
    ...
    def notify_func(self,city,state):
        for m, CC, RC, DC in zip(merged.index, merged["Confirmed cases"], merged["Recovered cases"], merged["Death toll"]):
                    if "Maharashtra" in m:
                        #print(CC, RC, DC)
                          frame_notify_ = "Maharashtra => | Total : " + str(CC) + " | Deaths : "  + str(DC) + " | Recovered : " + str(RC)
        ...
        for m1, CC_, RC_, DC_ in zip(city_data.index, city_data["Confirmed cases"], city_data["Recovered cases"], city_data["Death toll"]):
            if city in m1:
                print(CC_, RC_, DC_)
                frame_city_text = city + " => Total Cases : " + str(CC_) + "| Total Deaths : " + str(DC_) +  "| Recovered : " + str(RC_)  
